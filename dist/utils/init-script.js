 ...
 */
export function getInitScriptPath(cwd) {
 ...
 */
export function initScriptExists(cwd) {
 ...
 */
export function runInitScript(worktreePath, cwd) {
    const scriptPath = getInitScriptPath(cwd);
    ...
    try {
        ...
        return {
            success: false,
            error: `Failed to run init script: ${err.message}`,
        };
    }
}
/**
 ...
 */
export function warnNoInitScript(cwd) {
    const scriptPath = getInitScriptPath(cwd);
    if (!scriptPath)
        return;
    console.log(chalk.yellow(`\nTip: No ${INIT_SCRIPT_NAME} script found at ${scriptPath}`));
    console.log(chalk.dim(`Create this script to automatically run setup tasks (e.g., copy .env files) when creating new worktrees.`));
    console.log(chalk.dim(`Run 'wtm new --help' for more information.`));
}
/**
 * Run init script if it exists, otherwise show a warning.
 * This is the main entry point for commands that create worktrees.
 */
export function runInitScriptWithWarning(worktreePath, cwd) {
