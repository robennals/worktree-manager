 ...
 */
export function copyEnvToWorktree(
  worktreePath: string,
  options: CopyEnvOptions = {}
): void {
  const {
    sourcePath = ".env",
    subdirs = DEFAULT_ENV_SUBDIRS,
    verbose = true,
  } = options;
  ...
  if (!existsSync(sourcePath)) {
    if (verbose) {
      console.log(
        chalk.yellow(`Note: ${sourcePath} not found; skipping env copy.`)
      );
    }
    return;
  }
  ...
  for (const subdir of subdirs) {
    ...
    if (existsSync(targetDir)) {
      try {
        ...
        if (verbose) {
          console.log(
            chalk.red(`Failed to copy to ${targetFile}: ${(err as Error).message}`)
          );
        }
      }
    } else if (verbose) {
